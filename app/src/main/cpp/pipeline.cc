...
#include "opencv_puttext_zh/cv_text.h"
#include <jni.h>

std::shared_ptr<Utils::CvText> font_ptr_;
...
cv::Mat Visualization(cv::Mat srcimg,
                      std::vector<std::vector<std::vector<int>>> boxes,  std::vector<std::string> rec_text,
  ...
  for (int n = 0; n < boxes.size(); n++) {
    ...
    font_ptr_->putText(img_vis, rec_text[n], rook_points[n][0], cv::Scalar(255, 0, 0));
//    cv::putText(img_vis, rec_text[n], rook_points[n][0], font_face, font_scale, color,
//                thickness);
  }
//  cv::Mat img_vis_bgr;
...
Pipeline::Pipeline(const std::string &detModelDir,
  ...
  //std::string fontPath = "/sdcard/font/simhei.ttf"; // be careful to use the font that support Chinese

  const char* fontPath = "/sdcard/Android/data/com.baidu.paddle.lite.demo.object_detection/files/Download/fonts/jianti.ttf";
  font_ptr_.reset(new Utils::CvText(fontPath, Utils::CvText::UTF8));
  LOGE("fontPath1 font_ptr_: %s\n", fontPath);
...
bool Pipeline::Process_val(int inTextureId, int outTextureId, int textureWidth,
  ...
  // visualization
  auto img_res = Visualization(bgrImage_resize, boxes, rec_text, rec_text_score,savedImagePath);
  cv::Mat img_vis;
...
std::vector<OCR_INFO> Pipeline::Process_val(cv::Mat &rgbaImage)
{
  std::vector<OCR_INFO> rets(0);
  int height = 448;
  int width = 448;
  // change to 3-channel
  cv::Mat bgrImage;
  ...
  for (int i = 0; i < boxes.size(); i++) {
    ...
    OCR_INFO tmp_info;
    for (int j = 0; j < 4; ++j) {
      tmp_info.pts[j].x = boxes[i][j][0] * rgbaImage.cols / width;
      tmp_info.pts[j].y = boxes[i][j][1] * rgbaImage.rows / height;
    }
    tmp_info.text = res.first;
    tmp_info.score = res.second;
    rets.emplace_back(tmp_info);
  }
  return rets;
}