...
#include "Yolov5V_Pipeline.h"
#include <algorithm>
#include <map>
#include <utility>

Detector_Yolov5v::Detector_Yolov5v(const std::string &modelDir, const std::string &labelPath,
...
void Detector_Yolov5v::InitParams(const int &width, const int &height) {
  if (isInited_)
    return;
...
std::vector<std::string> Detector_Yolov5v::LoadLabelList(const std::string &labelPath) {
  std::ifstream file;
...
std::vector<cv::Scalar> Detector_Yolov5v::GenerateColorMap(int numOfClasses) {
...
void Detector_Yolov5v::Preprocess(const cv::Mat &rgbaImage) {
...
void Detector_Yolov5v::ExtractBoxes(int seq_id, const float *in,
...
void Detector_Yolov5v::Nms(const std::map<int, std::vector<Object>> &src,
...
void Detector_Yolov5v::Postprocess(std::vector<Object> *results) {
...
void Detector_Yolov5v::Predict(const cv::Mat &rgbaImage, std::vector<Object> *results,
                               double *preprocessTime, double *predictTime,
                               double *postprocessTime) {
  LOGD("Invoke AndroidBitmap_getInfo()s %f ms", *preprocessTime);
...
Yolov5V_Pipeline::Yolov5V_Pipeline(const std::string &modelDir, const std::string &labelPath,
                                   const int cpuThreadNum, const std::string &cpuPowerMode,
                                   int inputWidth, int inputHeight,
                                   const std::vector<float> &inputMean,
                                   const std::vector<float> &inputStd, float scoreThreshold) {
  detector_.reset(new Detector_Yolov5v(modelDir, labelPath, cpuThreadNum, cpuPowerMode,
                                       inputWidth, inputHeight, inputMean, inputStd,
                                       scoreThreshold));
}

void Yolov5V_Pipeline::VisualizeResults(const std::vector<Object> &results,
                                        cv::Mat *rgbaImage) {
...
void Yolov5V_Pipeline::VisualizeStatus(double preprocessTime, double predictTime,
...
bool Yolov5V_Pipeline::Process(cv::Mat &rgbaImage, std::string savedImagePath) {
...
std::vector<Object> Yolov5V_Pipeline::Process(cv::Mat &rgbaImage) {
  double preprocessTime = 0, predictTime = 0, postprocessTime = 0;

  // Feed the image, run inference and parse the results
  std::vector<Object> results(0);
  LOGE("Invoke AndroidBitmap_getInfo() RecDet::process %p!", detector_.get());
  detector_->Predict(rgbaImage, &results, &preprocessTime, &predictTime,
                     &postprocessTime);
