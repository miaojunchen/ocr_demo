...
Java_com_baidu_paddle_lite_demo_ppocr_1demo_Native_nativeInit_1yolov5v(
    ...
    return reinterpret_cast<jlong>(
            new Yolov5V_Pipeline(modelDir, labelPath, cpuThreadNum, cpuPowerMode, inputWidth,
...
Java_com_baidu_paddle_lite_demo_ppocr_1demo_Native_nativeRelease_1yolov5v(
        JNIEnv *env, jclass thiz, jlong ctx) {
    if (ctx == 0) {
        return JNI_FALSE;
    }
    Yolov5V_Pipeline *pipeline = reinterpret_cast<Yolov5V_Pipeline *>(ctx);
    delete pipeline;
    return JNI_TRUE;
}
 ...
 */
JNIEXPORT jboolean JNICALL
Java_com_baidu_paddle_lite_demo_ppocr_1demo_Native_nativeProcess_1yolov5v(
