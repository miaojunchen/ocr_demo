...
#include <cctype>
#include <utility>
#include "opencv_puttext_zh/tools.h"
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            ...
            if (slot->bitmap.buffer[off] & (0xC0 >> (j % 8))) {
                ...
                if (cur_g==0)
                {
                    printf("off:%d, i =%d, j = %d, r = %d, c = %d, pos:%d_%d\n", off, i, j, r, c, pos.x, pos.y);
                }
                cur_g++;
                ...
                if (r >= 0 && r < img.height && c >= 0 && c < img.width) {
                    ...
                    if (count_t==0)
                    printf("off:%d, i:%d, j:%d,r=%d, c=%d, v:%f,%f,%f,%f, wid:%d, hei:%d\n", off, i, j, r, c, scalar.val[0], scalar.val[1], scalar.val[2], scalar.val[3], img.width, img.height);
#else
                if (r >= 0 && r < frame.rows && c >= 0 && c < frame.cols) {
                    cv::Vec3b scalar = frame.at<cv::Vec3b>(r, c);
                    if (count_t==0)
                        printf("off:%d, i:%d, j:%d, r=%d, c=%d, v:%d,%d,%d, wid:%d, hei:%d\n", off, i, j, r, c, scalar[0], scalar[1], scalar[2], frame.cols, frame.rows);
...
#ifdef CV_1
                    for (int k = 0; k < 4; ++k) {
                        //printf("k_%d:%f, %f\n", k, scalar.val[k], color.val[k]);
                        scalar.val[k] = scalar.val[k] * (1 - p) + color.val[k] * p;
#else
                    for (int k = 0; k < 3; ++k) {
                        //printf("k_%d:%d, %f\n", k, scalar[k], color.val[k]);
                        scalar[k] = scalar[k] * (1 - p) + color.val[k] * p;
